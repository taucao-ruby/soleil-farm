...
/**
 * Request validation cho cập nhật nguồn nước.
 *
 * Validates:
 * - Tên và mã nguồn nước (unique trừ bản ghi hiện tại)
...
class UpdateWaterSourceRequest extends FormRequest
{
    ...
    {
        $waterSourceId = $this->route('water_source')?->id ?? $this->route('water_source');
        ...
        return [
            ...
            'code' => [
                ...
                'string',
                'max:30',
                Rule::unique('water_sources', 'code')->ignore($waterSourceId),
            ],
            'source_type' => ['sometimes', 'required', 'in:well,river,stream,pond,irrigation_canal,rainwater,municipal'],
    ...
    public function messages(): array
    {
        return [
            ...
            'source_type.in' => 'Loại nguồn nước không hợp lệ',
            ...
            'reliability.in' => 'Độ tin cậy không hợp lệ',
            'water_quality.in' => 'Chất lượng nước không hợp lệ',
