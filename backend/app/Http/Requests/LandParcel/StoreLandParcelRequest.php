...
/**
 * Request validation cho tạo mới lô đất.
 *
 * Validates:
 * - Tên và mã lô đất
 * - Diện tích và đơn vị
...
class StoreLandParcelRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'area_value' => ['required', 'numeric', 'min:0.01', 'max:999999.99'],
            'area_unit_id' => ['required', 'integer', 'exists:units_of_measure,id'],
    ...
    public function messages(): array
    {
        return [
            ...
            'land_type.in' => 'Loại đất không hợp lệ. Chọn: ruộng lúa, vườn, ao, đất tổng hợp, đất nghỉ, khác',
            ...
            'terrain_type.in' => 'Loại địa hình không hợp lệ. Chọn: bằng phẳng, dốc, bậc thang, vùng trũng',
            'soil_type.in' => 'Loại thổ nhưỡng không hợp lệ. Chọn: đất sét, đất cát, đất thịt, đất phù sa, đất pha',
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (empty($this->code)) {
            $count = LandParcel::count() + 1;
            $this->merge([
                'code' => 'LD-' . str_pad($count, 4, '0', STR_PAD_LEFT),
            ]);
        }
