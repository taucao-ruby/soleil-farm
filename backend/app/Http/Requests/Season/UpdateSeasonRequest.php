...
/**
 * Request validation cho cập nhật mùa vụ.
 */
class UpdateSeasonRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $definitionId = $this->season_definition_id ?? ($season instanceof Season ? $season->season_definition_id : null);
            $year = $this->year ?? ($season instanceof Season ? $season->year : null);

            // Check for duplicate season (same definition + year) excluding current
            if ($definitionId && $year) {
                $exists = Season::where('season_definition_id', $definitionId)
                    ->where('year', $year)
                    ->where('id', '!=', $seasonId)
                    ->exists();
                ...
                if ($exists) {
                    $validator->errors()->add(
                        'year',
                        'Mùa vụ này đã tồn tại cho năm ' . $year
                    );
                }
            }
        });
